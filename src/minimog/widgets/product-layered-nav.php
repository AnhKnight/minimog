...
if ( ! class_exists( 'Minimog_WP_Widget_Product_Layered_Nav' ) ) {
	class Minimog_WP_Widget_Product_Layered_Nav extends Minimog_Widget {
		...
		public function __construct() {
			...
			$this->widget_cssclass    = 'minimog-wp-widget-product-layered-nav';
			$this->widget_name        = esc_html__( '[Minimog] Product Attribute Layered Nav', 'minimog' );
		...
		protected function layered_nav_dropdown( $terms, $taxonomy, $query_type ) {
			...
			if ( $taxonomy !== $this->get_current_taxonomy() ) {
				...
				$any_label            = apply_filters( 'woocommerce_layered_nav_any_label',
					sprintf( esc_html__( 'Any %s', 'minimog' ), $taxonomy_label ),
					$taxonomy_label,
					$taxonomy );
				...
				echo '<select class="dropdown_layered_nav_' . $taxonomy_filter_name . '" data-filter-url="' . preg_replace( '%\/page\/[0-9]+%',
						...
						str_replace( array(
							...
							esc_js( add_query_arg( 'filtering',
								...
								) ) ) ) ) ) . "&filter_" . esc_js( $taxonomy_filter_name ) . "=minimog_FILTER_VALUE" . '">';
		...
		protected function layered_nav_list( $terms, $taxonomy, $query_type, $instance ) {
			...
			foreach ( $terms as $term ) {
				 ...
				 */
				$term_html = apply_filters( 'minimog_woocommerce_layered_nav_term_html', $term_html, $term, $link, $count );
